...
-- ═══════════════════════════════════════════════════════════
-- ENHANCED GUN HOOK (SHERIFF SAFE TARGETING + KALMAN PREDICTION)
-- ═══════════════════════════════════════════════════════════
--[[
    This hook integrates:
    1. Sheriff Safe-Targeting (LOS checks, role verification)
    2. Advanced prediction from AimAssist module (Kalman/EMA)
            ...
            if self.Name == "RemoteFunction" and method == "InvokeServer" then
                ...
                if localRole == "Sheriff" and EnhancedConfig.SheriffSafeTargeting.Enabled then
                    ...
                    if murderer and murderer.Character then
                        -- Step 1: Check if we can safely target the murderer (LOS check)
                        if SymphonyEnhanced.TargetingSystem.sheriffTargeting:canTargetMurderer(murderer, playerData) then
                            -- Step 2: Use advanced AimAssist prediction instead of simple math
                            
                            -- Lock onto murderer if not already locked
                            if not aimAssist:isLocked() or aimAssist:getTarget() ~= murderer then
                                aimAssist:lock(murderer)
                            end
                            
                            -- Get predicted position from AimAssist (uses Kalman/EMA/Latency/Jump prediction)
                            local predictedPosition = aimAssist:getPredictedPosition()
                            
                            if predictedPosition then
                                -- Use advanced prediction
                                args[2] = predictedPosition
                            else
                                -- Fallback: Use simple prediction if AimAssist fails
                            ...
                            -- LOS check failed - block the shot to prevent friendly fire
                            return nil
                        end
                    end
                end
