...
local loadSuccess, loadResult = pcall(function()
    return loadstring(game:HttpGet("https://cdn.jsdelivr.net/gh/AlphaZero-Chess/V@refs/heads/main/Symphony_Refactored.lua"))()
end)

if not loadSuccess then
    warn("[Symphony] Failed to load core: " .. tostring(loadResult))
    error("Symphony core loading failed!")
end

-- Wait for Symphony to be available in global scope
local maxWait = 5
local waited = 0
while not _G.Symphony and not Symphony and waited < maxWait do
    task.wait(0.1)
    waited = waited + 0.1
end

-- Check if Symphony is available (either as global or from loadResult)
local Symphony = _G.Symphony or Symphony or (loadResult and loadResult.Core)

if not Symphony then
    error("Symphony core not loaded after " .. maxWait .. " seconds!")
end

print("[Symphony][INFO] Symphony core verified and loaded")
print("[Symphony][INFO] Version: " .. (Symphony.getVersion and Symphony:getVersion() or "Unknown"))
